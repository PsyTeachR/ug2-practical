...
```{r, echo=FALSE, results = 'asis'}
cat("You will use the Wickham Six very frequently for wrangling your data so this would definitely be something you should be making notes about - not just the names, but how they work and any particular nuances that you spot.
")
```
`r unhide()`   
...
```{r, echo=FALSE, results = 'asis'}
cat(" What does `desc()` do? Compare the output of the two lines above on the `BallSpeed` column. Does `desc()` also work for `BackgroundColor`?
...
`r hide("Helpful Hint")`
```{r, echo = FALSE, results='asis'}
cat("It is the same procedure as this but with different column names:
