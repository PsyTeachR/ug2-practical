...
1. Download the data as a zip file from [this link](data/02-s01/preclass/Level2_Lab2_InclassData_Pong.zip) and save it somewhere you have access. In the lab, use your `M:` drive.

2. Set your working directory to the same folder as the data. `Session >> Set Working Directory >> Choose Directory`

3. Open a new script and copy and paste the two lines below. Here we are a) loading the `tidyverse` from our package library into our session and then b) loading in the data through the `read_csv()` function and storing it in the tibble called `pong_data`.
...
<!-- one issue here is that read_csv() defaults to loading in numeric columns
     as type 'double', so the data types in the table are not accurate.
     I suggest changing read_csv() to use the col_types argument 
     (as I have done above). -DJB -->

Note that we have specified the `col_types` argument as `"iiiiicii"`. The `col_types` argument allows you to control the data type for each variable. If you don't specify this argument, the default is for `read_csv()` to guess, and when it sees numbers in a column, it will default to treating it as type `double`.
...
<!-- 
I am not a fan of overwriting a table. Can lead to confusion if lines are run in a jumbled order, as students often do. Also, it could feed into the false impression that students often have that functions such as mutate, filter, select etc. actually alter the original data rather than creating an altered copy of the dataset.
-->
...
You can only renumber the trials **for each participant** if you group the tibble by the values in `Participant` before applying the `mutate()` operation.

* mutate(`TrialNumber` = TrialNumber minus 1)
```
`r unhide()`  

```{r hidden-block-1, include=FALSE}
pong_data2 <- filter(pong_data, TrialNumber >= 2) %>%
  mutate(TrialNumber = TrialNumber - 1L)
```
...
* Run the first two lines of code in the box above to create `pong_data2_hits` and then enter the number of hits made with the small paddle (50) and the red color background in this box: `r fitb(pong_data2_hits_red_small %>% pull(total_hits))`

<!-- answer you gave was incorrect (517 rather than 516) because the code you had that overwrote pong_data was not executed and practice trials were still being included. Better to generate answer dynamically -DJB -->
...
pong_data2_hits_red_small <- pong_data2 %>% 
