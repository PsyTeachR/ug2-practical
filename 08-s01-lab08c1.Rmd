...
Today we will use the function `pwr.t.test()` to run our calculations. This is a function available in **`pwr`**. If you are using the Boyd Orr machines the `pwr` package is already installed and you will just need to call it like all other packages, e.g. `library(pwr)`. Do not attempt to install it yourself on the Boyd Orr machines. If you are using your own laptop then feel free to install it. If for some reason you can't install the **`pwr`** library there is a similar function in base R called `power.t.test()` that you can use without installing a library. There is an example of this function in the additional materials of this chapter. However we do recommend using the `pwr.t.test()` function where possible and that is what we will use here.

Remember that for more information on this function, simply do `?pwr.t.test` in the console window after loading in the library. On doing this you will see that `pwr.t.test()` takes a series of inputs:
...
* **d** - the difference between means, the effect size (Cohen's d)
* **sig.level** or $\alpha$ - the false positive rate (Type 1 error)
* **power** or $1-\beta$
...
Using a pipeline, store the answer as a single value called `sample_size` (e.g. think `tidy() %>% pull()`) and round up to the nearest whole participant. You will probably need to use `ceiling()` instead of `round()`. For example, `ceiling(1.1)` gives you `r ceiling(1.1)`. `ceiling()` always rounds up! `floor()` is the opposite of `ceiling()`.
...
* This time we know everything except the effect size - Cohen's d.
* You will also need to use `tidy()`, `pull()` to obtain Cohen's d, the effect size, and `round()` so the value is rounded to two decimal places.
```
`r unhide()`  
<br>
...
* Calculate the achievable Cohens d for the studies and then calculate the established Cohen's d for the studies. Remember you know the $power$ and $\alpha$ values already
```
`r unhide()`
<br>
