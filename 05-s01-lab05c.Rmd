...
4. Locate the difference you observed in step 1 (the real difference) on the null hypothesis distribution of differences. 
...
1.2.  Now type the statement `set.seed(1011)` at the top of your script after your library call and run it.  (This 'seeds' the random number generator so that you will get the same results as everyone else. The number 1011 is a bit random but if everyone uses it then we all get the same outcome. Different seeds give different outcomes. Also if you want to get the exact results shown here, make sure you are using R version 3.6 or higher, because random number generation changed from R version 3.5 to 3.6.) 

1.3.  [Download the data file from here](data/05-s01/inclass/perm_data.zip) and read the data in `perm_data.csv` into a variable called `dat`.  
...
We now need to write a pipeline of five commands that calculates the mean difference between the groups in `dat`, Group A minus Group B. Broken down into steps this would be: 
