...
**Note:** When writing a code or your own report, you can make your write-up reproducible as well by using the output of your tibbles and calling specific columns. For example, t(`results$parameter`) = `results$statistic %>% abs()`, p < .001, when converted to inline coding and knitted will become t(`r results$parameter`) = `r results$statistic %>% abs()`, p < .001. So code can prevent mistakes in write-ups!
