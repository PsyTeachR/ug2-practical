...
* The `n` NULL relates to: `r mcq(c(answer = "Number of replications", "Number of blocks in the experiment", "Number of trials in the experiment", "Number of faces in the experiment"))`
...
You saw this exact same code in the preclass so be sure to have had a look there. The solution code is shown in one of the below code chunks.
```
`r unhide()`
...
* Using the appropriate `binom()` function, to three decimal places, type in the actual probability of 10 faces in 60 trials: `r fitb(c("0.002",".002"), width = 8, ignore_ws = TRUE)`
...
* Using the appropriate `binom()` function, type in what would be the minimum number of faces needed in a block of 60 trials to maintain at least 5% of trials being faces.`r fitb(c("14"), width = 8, ignore_ws = TRUE)`
