...
* You want to show the code you are running in your analysis but not show the output as this would be too much to display. Note that you want the code to run. Type in the box (e.g. `option = value`) how you would set the `results` option to do this? `r fitb("results = \"hide\"", width = 20, ignore_ws = TRUE)`
...
* True or False, writing `echo = TRUE` has the same effect on the output of a chunk as if you had no echo rule at all: `r torf(TRUE)`

`r hide("Explain This - Echo True or Not at all")`
```{block, type = "info"}
All of the options have a default value. For example, `echo`, `include`, and `eval` are usually by default set to `TRUE`. As a result, if you don't set the `echo` option, i.e. you don't declare `echo = FALSE`, then it is the same as declaring `echo = TRUE`. So not specifying an option will give you the default value for that option.
```
`r unhide()`  
<br>  

* True or False, there is no difference between the chunk options `results = "hide"` and `eval = FALSE` as they both hide the output: `r torf(FALSE)`

`r hide("Explain This - What's the difference?")`
```{block, type = "info"}
With the first option, `results = "hide"`, the code is evaluated and results are produced but the output is hidden. With the second option, `eval = FALSE`, the code is not evaluated and therefor no results or output has been produced. If you need your output for a later part of the code then you would use the `results` option. If you don't need the output and just want to show the code as an example then you would use the `eval` option.
```
`r unhide()`  

### Adding Inline Code

An alternative way to add code to a report is through what is called **inline code**. Inline code is called this because it appears outside of a chunk, i.e., within text. Inline coding can be inserted using a back-tick, then the letter `r`, followed by a space, the code you want to run, then another back-tick. For example, writing `r backtick("r 2 + 2")` would return the answer **4** when you knit the file instead of showing the code. Note that you do not do this inside a code chunk, you do this inline with your text, e.g.:
<br><br>
