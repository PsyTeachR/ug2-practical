...
* Giving a n_participants value of `r n_participants`
...
* For `ns_data_sd` use `summarise()` to calculate the mean and then `pull()` the value.
```{r mean_sd, eval  = TRUE}
# the mean
...
And then combine all the information in to one aingle tibble.
...
When formatted to with inline R code as below:
