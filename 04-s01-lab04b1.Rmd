...
**3.** Now say you have two standard packs of cards **mixed** together. The probability of drawing the 10 of clubs from this mixed pack would be 2 in 104: $p = \frac{2}{104}= .019$. Two possible outcomes **but more alternatives** than above, 104 this time.

<!-- 
above point said it was "less likely" but the probability is actually the same as in #1
-->

**4.** Let's instead say you have two **separate** packs of cards. The probability of drawing the 10 of clubs from **both** packs would be: $p = \frac{1}{52} \times \frac{1}{52}= .0004$. The probability has gone down again because you have created an event that is **even more unlikely** to happen - this is called the `joint probability` - multiplying the two individual probabilities of **two independent, mutually exclusive events** to find the joint probability. 

<!--
Cumulative probability != joint probability
-->
...
```{block, type ="info"}
A sampling distribution shows you the probability of drawing samples with certain characteristics; e.g. the probability of 5 heads in 10 flips, or the probability of 4 heads in 10 flips, or the probability of X heads in 10 flips of the coin.  
```
`r unhide()`  
<br>
...
Note that as the possible number of heads in 10 flips are all related to one another, then summing up all the probabilities of the different number of heads will give you a total of 1. You can use this informaiton to start asking questions such as what would be the probability of obtaining 2 or less Heads in 10 flips?  Well, if the probability of no heads in this distribution is **`r format(data10k$p[1], scientific = FALSE)`**, and the probability of 1 heads is **`r data10k$p[2]`**, and the probability of 2 heads is **`r data10k$p[3]`**, then the probability of 2 or less Heads in this distribution is simply the sum of these values: **`r data10k$p[1] + data10k$p[2] + data10k$p[3]`**
...
As such the probability of 6 heads, using `dbinom` is p = `r dbinom(6, 10, 0.5)`. If you compare this value to the `data10k` value for 6 you will see they are similar but not quite the same because `dbinom` uses a lot more replications than the 10000 we used. 
...
* Using the format of the above `pbinom` function, enter the code that would determine the probability of **up to and including** 5 heads out of 20 flips, assuming a probability of 0.5: `r fitb(c("pbinom(5, 20, 0.5)","pbinom(5, 20, .5)","pbinom(5, 20, 0.5, lower.tail=TRUE)", "pbinom(5, 20, .5, lower.tail=TRUE)"), width = 45, ignore_ws = TRUE)`
...
<!--
This section is confusing. I have tried to clarify. -DJB
-->
...
Let's see how these functions are inverses of one another:

```{r inv-1-pbinom}
p1 <- pbinom(49, 100, .5) # probability of 0 to 49 heads in 100 coin flips
p1

qbinom(p1, 100, .5) # gives us the value of 'x' that cuts off a probability of p1
```

For example, if we want to know what value cuts off a lower tail probability of .05, we would use the following code.
...
which is not exactly .05. This is because `x` is discrete, and so `qbinom()` gives us the closest category to the boundary.
...
First, it is useful to remember that it is the inverse of `pbinom()`: `pbinom()` gives you a tail probability associated with `x`, and `qbinom()` gives you the closest `x` that cuts off the specified tail probability.
...
Remember that you want an overall probability of 10% (`p` = .1), you have 17 flips in each go (`size` = 17), and the probability of heads on any one flip is .5 (`prob` = .5). And you the maximum number for the lower tail, so the `lower.tail` is TRUE.
```
`r unhide()`
<br>
...
<!-- 
I still think this section is confusing. We probably should revisit and try to clarify.
-DJB
-->
