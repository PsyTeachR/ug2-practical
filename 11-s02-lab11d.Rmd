...
```{r dmx_warning, echo = FALSE}
# Don't change anything in this code chunk; it is just here to help you check
# that you've defined a table called `dmx` correctly. Pay attention to any warnings
# that appear when you run this chunk. When the warning has gone away, that
# means that the structure of `dmx` is correct (although it doesn't mean that
# the values are correct!)
if (!identical(names(dmx), names(dsetup)) || 
    !identical(dim(dmx), dim(dsetup))) {
  warning("If this warning appears in the output, you have not yet defined 'dmx' properly, and will lose points on the assessment. Check column names (including capitalization), column data types, and table structure.")
}
```
