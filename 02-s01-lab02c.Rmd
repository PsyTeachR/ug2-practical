...
```{r, echo = FALSE, results='asis'}
cat("We use `Tidy Data` because it is really efficient and works well with the `tidyverse`. However, people used to use data structured in `long` format or `wide` format. 
...
<br>
**[Click here to download](https://www.psy.gla.ac.uk/~phil/L2labs/Lab2Semester1/AQ_Analysis_Files.zip)** the files as a zip file. Alternatively download them from the Lab2 Moodle tab. Now unzip the files into a folder on your `M:` drive. We will use zip folders a lot so if this is something you struggle with please ask.

`r hide("Portfolio Point - Open Data is best in csv format")`
```{r, echo = FALSE, results='asis'}
cat("`csv` stands for 'comma separated variable', and is a very basic way of transferring data. It really just stores numbers and text and nothing else. The great thing about being this basic is that it can be read by many different machines and does not need expensive licenses to open it.")
```
`r unhide()`
...
```{r, echo = FALSE, results='asis'}
cat("The answer to all of these are True. 
...
```{r, echo = FALSE, results='asis'}
cat("combine the function `library()` and the package `tidyverse` and remember that the solutions are at the bottom of the page.
...
```{r, echo = FALSE, results='asis'}
cat("As you work with data and functions you will find there are functions with similar names but that give different results. One of these is the `read` function for `csv`. Make sure to always use `read_csv()` as your function to load in `csv` files. Nothing else. It is part of the `readr` package automatically brought in with `tidyverse`.")
```
`r unhide()`
...
```{r, echo = FALSE, results='asis'}
cat("The `reponses` tibble is far from being tidy; each row represents multiple observations from the same participant, i.e. each row shows responses to multiple questions - `wide format`. Remember we want the data in tidy format as described above.
...
```{r, echo = FALSE, results='asis'}
cat("sandwich <- gather(responses, Jam, Strawberry, Q1:Q3)
...
```{r, echo = FALSE, results='asis'}
cat("We saw Pipes in Level 1 and then again in the Preclass for this lab. Pipes are your friend. Think of them as saying 'and then' or 'goes into'. So in the example above we take `rscores` and then group it by something and then summarise it into AQ scores based on... 
