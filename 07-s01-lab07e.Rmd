...
```{r ch7-task5-2, results='asis', fig.cap="Alternative Solution to Task 5"}
lates %>% 
  ggplot(aes(x=Timepoint,y=GuiltRating))+
  geom_violin(aes(fill = Timepoint), alpha = .2) + 
  geom_boxplot(width = 0.5) +
...
Which when run, would return as p < `r ifelse(round(results$p.value,3) == 0, .001, round(results$p.value,3))`
