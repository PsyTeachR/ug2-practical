...
.n_onesamp <- power.t.test(delta = .23, sd = 1, power = .8, 
                           sig.level = .05, 
                           alternative = "two.sided", 
                           type = "one.sample") %>% 
  tidy() %>%
...
<!--
I don't think this analysis is valid. You can't say based on a study outcome that a single study was appropriately powered or not. Also there is noise around the effect size estimate so even if you get .5 the real effect may be larger. I think we should focus on the use of power in study planning, not in retrospectively evaluating a study
-->
...
<!-- Again, I worry a little here about post-hoc power calculations. Hmm -->

Thus far we have used hypothetical situations - now go look at the paper on the Open Stats lab website called <a href="https://sites.trinity.edu/osl/data-sets-and-activities/t-test-activities" target = "_blank">Does Music Convey Social Information to Infants?</a> - we looked at it a little in the lectures. You can download the pdf and look at it, but here we will determine the smallest effect size we can detect for a given alpha level and power. There is a one-sample t-test and a paired-samples t-test to consider, summarised below. Assume testing was at power = .8, alpha = .05 and calculate the minimum effect size you could detect given those values. <!-- "Based on your calculations are either of the stated effects underpowered?" effects can't be underpowered, only studies/tests can, and power for a single study is not knowable with precision -->

<!-- 
What you are doing is fixing the sample size and solving for the smallest effect size you could detect with power > .8. That much I get, however:

It is invalid to then look at the effect size estimate and say, "well, actually the estimate we got is smaller than this so the study is underpowered."" The effect size will fall in some range depending on sampling/measurement error and the true population effect size is still unknown. I'm not sure what is gained by comparing the 'achievable' to the 'estimated' effect size.
-->
...
* Calculate the achievable Cohen's d for the studies
```
<!--
...
<!--
this again assuming that you can evaluate the 'power' of a single study, which is dubious
