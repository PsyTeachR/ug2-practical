...
<!--
The probability of any specific value (including exact zero) in a continuous distribution is infinitesimal, because there are an infinite number of points. This is how continuous distributions differ from discrete distributions. So I have deleted the next question because it is misleading.
-DJB
-->
...
The `pnorm()` function works just like `pbinom()`. So, `pnorm(x)`, given the  `mean` and `sd` of our data, returns a tail probability (or "cumulative probability") associated with `x`. The default is to give the probability of `x` or lower (the default is `lower.tail = TRUE`). If you want the probability of `x` or higher, use `lower.tail = FALSE`.
