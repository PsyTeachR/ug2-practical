...
```{block, type ="info"}
This value appears in column `err`, where `i` equals 3 and `j` equals 2. This can be thought of the unique difference for that participant from the sample mean and group mean.
```
`r unhide()`
<br>
...
`r hide("Step 1 Hints")`
```{block, type ="info"}
* You will need some wrangling functions so on't forget to load the tidyverse
