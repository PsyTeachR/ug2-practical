...
```{r boxplot-hide, fig.cap="A boxplot of our data to check for homogeneity of variance"}
...
One issue with the above example is that it requires another package - **`car`**. This isn't a major issue but say you only had the **`afex`** package to use. Well fortunately that package also has a function that can check Levene's Test - `afex::test_levene()`. And it works as follows:
