...
cat("Remember that the fewer the number of hashtags the larger the heading size.
")
```
`r unhide()`  
...
2. Download the data for this lab in a zip file by clicking [this link](http://www.psy.gla.ac.uk/~phil/L2Labs_201819/semester_one/lab_01/inclass/PonzoAgeData.zip). Unzip it and save it to the folder you are working in. 
...
**Knit the document now and see what the output looks like. It will ask you to save the file somewhere. Remember that on the Lab PCs this is best done on your M: drive, given space.**  

**Important**: There is a good chance that on the webpage that you have knitted, that you will see either some warnings or messages. You can suppress these using the `message` and `warning` rules within the code chunks as well. Try this now - the R-Markdown cheatsheet will help.
...
**1.** Loads the `tidyverse` packages and all associated packages e.g. `dplyr`, `readr` and `ggplot2` (**You learned these last year!**).  
...
* `read_csv()` is actually in the `readr` package and comes in with `tidyverse`. Always use this to read in data from a csv file.
...
1. Underneath the code chunk you entered, put a new heading called `Introduction` and set it as heading style 2
2. Do a little research with your group on the **Ponzo Illusion** and write a sentence or two describing its function; include a citation to support your research. There is a link to the wikipedia page at the top of this lab which might help. Just a sentence or two.
3. Finally, copy the following text into your report and below it put the names of two hypotheses behind the illusion in an **ordered list** style; i.e. 1... 2..., etc. The two hypotheses are **The Framing hypothesis** and **The Perspective hypothesis**. 

> **"There are two underlying hypotheses that may explain the Ponzo Illusion. These are: ..."**

In your group, try out the following questions to remind you about using citations:
...
For more information on how to format citations, you can look at this helpful [APA formatting and style guide](https://owl.english.purdue.edu/owl/resource/560/03/).  

### Task 5: Making Text Bold or Italicized

1. In your report, format the line `There are two underlying hypotheses...` in bold. **It's a good idea to `knit` the file at this point to make sure it is working correctly.**  
...
1. Create a new heading below your list of the two hypotheses and call it `Methods`. Set it as Header style 2. 
2. Now below `Methods` write a new heading called `Data` and set it as Header style 3.
3. Underneath this heading, copy the following sentence and turn the citation into a link to the paper. 
...
For certain studies you may want to add an image to the Methods section, either of the stimuli, of the materials, or of the procedure. If you look at the R Markdown cheatsheet you'll see that adding an image is very similar to adding a link, the only difference is the `!`. For now, we will just add an image of the illusion taken from the internet to illustrate how to do this.

1. Below the sentence you added for Task 6, add a new heading called `Stimuli` and set it as Header style 3.
2. Below the `Stimuli` heading, insert the image at the following web address:

```{r, echo = FALSE, results = 'asis'}
cat("https://upload.wikimedia.org/wikipedia/en/8/89/Ponzo_Illusion.jpg")
```
<br>
...
Another benefit of R Markdown is that you can insert tables of results directly into your report without having to format them - though for aesthetical purposes you will want to eventually learn how to do this as well though.

1. Create a new heading below your methods sentence, called `Results` and format it as Header style 2.
2. Add a new code chunk, give it the name `table` and include the code below. 
...
```{r, echo = FALSE, results = 'asis'}
cat("
Use RStudio's autocomplete (the tab button) to see the different options for include. Do this by typing `include =` and then hitting the tab button. You should see the options of `TRUE` or `FALSE`. This works for a lot of codes as well and functions you can't quite remember how to spell.
    ")
```
`r unhide()`
...
**Again, knit your document to make sure it is working correctly. In the Plot section, you should have the code followed by the nice scatterplot. We will learn how to improve the visualisations as we progress. Your document is now the final report!. You can compare it to the one we have created to see if they match. Fix anything that is not formatted as in our template.**  

`r hide("Portfolio Point - the ggplot package")`
```{r, echo = FALSE, results = 'asis'}
cat("
...
The code above uses the `ggplot2` package you used last year. This is the main package we use for plots, figures, visualisations, or however you like to call them. It can be called into the library by itself, or is automatically called in when you call in the `tidyverse` package. Later this semester we will revist `ggplot2` in more detail. For now, we are using it to make a scatterplot (`geom_point`) of Age (`Mean_Age`) and Comparison Length (`ComparisonLength`), and splitting the data for males and females.
    ")
```
`r unhide()`  
