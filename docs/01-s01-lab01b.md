...
### Adding Code Chunk Options

It can often be a good idea or even necessary to show the data or the outcome of a test in your report, for example if you were writing a report and wanted to include a table of results. But what if your code displayed a table that was 10,000 lines long? In that case we might want to not show the output and only show the code. You can do this by including an option within the first line of your code chunk - your \`\`\`{r name, option = value} line. You have already seen a chunk option before in the standard default chunk, the `include` option, but there are a number of others.  

**To hide the output but show the code** we use the `results = "hide"` option:
...
<br>
**Add this option into your example code chunk, as shown above, and knit the file again. What happens?** Note that there is a comma separating the name of the chunk and the option. You should now see the code only and not the data.
<br>
Alternatively, we can **Hide the code, but show the output** by using the `echo = FALSE` chunk option:
...
<br>
In your template Rmd file the option echo is set to `FALSE` meaning to show the figure and not the code. **Change the option in your code to `echo` and set it as `TRUE`, then knit the file again**. What happens?  
...
<br>
You might want to **hide both the code AND the output** by using the `include` option:
...
<br>
**Change the option to your example code chunk, as shown above, to `include = FALSE` and then knit the file again**. What happens? Note that here the code still runs. It just does not show you anything. 

Finally, you can use the `eval` option which specifies whether or not you want the code chunk you have written to be evaluated when you knit the RMarkdown file. Evaluated means to run or carry out the code. Here, the `eval = FALSE` rule will stop the code from being evaluated. The code will be shown because there is no rule stopping it but there will be no output because it won't get evaluated because of the `eval` option being `FALSE`. 
...
* You want to show the code you are running in your analysis but not show the output as this would be too much to display. Note that you want the code to run. Type in the box (e.g. `option = value`) how you would set the `results` option to do this? <input class='solveme nospaces' size='20' data-answer='["results = \"hide\""]'/>
...
* True or False, writing `echo = TRUE` has the same effect on the output of a chunk as if you had no echo rule at all: <select class='solveme' data-answer='["TRUE"]'> <option></option> <option>TRUE</option> <option>FALSE</option></select>


<div class='solution'><button>Explain This - Echo True or Not at all</button>

<div class="info">
<p>All of the options have a default value. For example, <code>echo</code>, <code>include</code>, and <code>eval</code> are usually by default set to <code>TRUE</code>. As a result, if you don’t set the <code>echo</code> option, i.e. you don’t declare <code>echo = FALSE</code>, then it is the same as declaring <code>echo = TRUE</code>. So not specifying an option will give you the default value for that option.</p>
</div>
...
* True or False, there is no difference between the chunk options `results = "hide"` and `eval = FALSE` as they both hide the output: <select class='solveme' data-answer='["FALSE"]'> <option></option> <option>TRUE</option> <option>FALSE</option></select>


<div class='solution'><button>Explain This - What's the difference?</button>

<div class="info">
<p>With the first option, <code>results = &quot;hide&quot;</code>, the code is evaluated and results are produced but the output is hidden. With the second option, <code>eval = FALSE</code>, the code is not evaluated and therefor no results or output has been produced. If you need your output for a later part of the code then you would use the <code>results</code> option. If you don’t need the output and just want to show the code as an example then you would use the <code>eval</code> option.</p>
</div>
...
An alternative way to add code to a report is through what is called **inline code**. Inline code is called this because it appears outside of a chunk, i.e., within text. Inline coding can be inserted using a back-tick, then the letter `r`, followed by a space, the code you want to run, then another back-tick. For example, writing <code>&#096;r 2 + 2&#096;</code> would return the answer **4** when you knit the file instead of showing the code. Note that you do not do this inside a code chunk, you do this inline with your text, e.g.:
<br><br>
