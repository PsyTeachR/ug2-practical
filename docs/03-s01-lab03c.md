...
### Task 1: Loading and Viewing the Data
...
2. `head()`, `tail`, and `ls()` - These show the top six and bottom six rows with column names, or just the column names with `ls()`.
...
* From the options, what type of data is the variable `Angle`, found in the dataframe `menrot`? <select class='solveme' data-answer='["integer"]'>
    <option></option>
...
* From the options, which of these is a column within `menrot`? <select class='solveme' data-answer='["CorrectResponse"]'>
    <option></option>
    <option>Correctresponse</option>
    <option>CorretcResponse</option>
    <option>CorrectResponse</option>
    <option>correctReponse</option>
</select>


* From the options, according to the dim() call, how many rows are there in demog? <select class='solveme' data-answer='["54"]'>
    <option></option>
    <option>3</option>
    <option>8</option>
    <option>54</option>
...
3. `ls(menrot)` will give you column names and this is about making sure of the correct spelling: CorrectResponse.
...
2. Replace the `NULL`s in order to recreate the figure below similar to that of Ganis and Kievit (2016) Figure 2 (top). 
    ...
    * You can see if you can figure out what `coord_cartesian` does by changing the numbers another time.
...
The first four lines, to create `menrot_time_angle`, are all functions from the Wickham Six verbs so refer back to your portfolio to see how they work.
...
<img src="03-s01-lab03c_files/figure-html/unnamed-chunk-7-1.png" alt="**CAPTION THIS FIGURE!!**" width="100%" />
...
* From the options, the figure would suggest that as angle of rotation increases: <select class='solveme' data-answer='["mean reaction time increases"]'>
    <option></option>
    <option>mean reaction time decreases</option>
    <option>mean reaction time stays the same</option>
    <option>mean reaction time increases</option>
</select>
...
### Task 3: Examining Additional Variable Effects
...
<br>
<div class="figure" style="text-align: center">
<img src="03-s01-lab03c_files/figure-html/unnamed-chunk-10-1.png" alt="**CAPTION THIS FIGURE!!**" width="100%" />
...
### Task 4: Grouping the Figure Data
...
### Task 5: Identifying Groups Using **`aes()`**
...
<div class="figure" style="text-align: center">
<img src="03-s01-lab03c_files/figure-html/unnamed-chunk-13-1.png" alt="**CAPTION THIS FIGURE!!**" width="100%" />
...
### Task 6: Changing the Shape and Size of Data Points
...
<div class="figure" style="text-align: center">
<img src="03-s01-lab03c_files/figure-html/unnamed-chunk-15-1.png" alt="**CAPTION THIS FIGURE!!**" width="100%" />
...
`size` is called outside of the `aes()` and we assign it a specific value. As you can see from the Task 6 figure, each condition has now the same size of points. We could set this size to what we want but keep it smallish: 3 or 5 are ok; 50 would be artistic but not that informative.
...
So hopefully this is starting to make sense and you can think about implementing it in your own figures. Note that arguements are separated by a comma. e.g. `geom_point(color = "red", size = 3, shape = 2)
    

</div>


### Task 7: Adding Labels and Changing the Background
...
The key thing is to remember to + the layer into the ggplot chain. And don't get this confused with pipes (%>%). You add (+) layers, and you pipe (%>%) functions.
    

</div>


### Task 8: Separating a Variable and Removing Legends
...
<div class="figure" style="text-align: center">
<img src="03-s01-lab03c_files/figure-html/unnamed-chunk-19-1.png" alt="**CAPTION THIS FIGURE!!**" width="100%" />
...
* In both sex, mean reaction time <select class='solveme' data-answer='["increases with"]'>
    <option></option>
    <option>decreases with</option>
    <option>increases with</option>
    <option>is unaffected by</option>
</select>
 angle of rotation.

* Angle of Rotation influences <select class='solveme' data-answer='["female participants"]'>
    <option></option>
    <option>female participants</option>
    <option>male participants</option>
</select>
 more than <select class='solveme' data-answer='["male participants"]'>
    <option></option>
    <option>female participants</option>
    <option>male participants</option>
</select>
...
Also, as the male mean reaction times are overall quicker than the female mean reaction times, and the differences in reaction times between 0 degrees and 150 degrees is smaller in males, then you could perhaps say that males are affected less than females, or males perform the task quicker.
