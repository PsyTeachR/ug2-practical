...
* From the options, what type of data is the variable `Angle`, found in the dataframe `menrot`? <select class='solveme' data-answer='["integer"]'> <option></option> <option>character</option> <option>integer</option> <option>double</option></select>
...
<li><p>calling <code>str(menrot)</code> and looking through the information that comes out you should see that the data in the column <code>Angle</code> are integers (whole numbers with no decimal places).</p></li>
