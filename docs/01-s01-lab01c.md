...
<li><code>read_csv()</code> is actually in the <code>readr</code> package and is available to you only after you have loaded in <code>tidyverse</code> through <code>library(tidyverse)</code>. We will always tell you to use <code>read_csv()</code> to read in data from a csv file.</li>
