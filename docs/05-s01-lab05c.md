...
Note: This lab was written under R version 3.5.1 (2018-07-02). If you use a different RVersion (e.g. R 3.6.x) or set a different seed (e.g. `1509`) you may get different results.
...
* Check that your value for $D_{orig}$ is correct, without using the solution, by typing your $D_{orig}$ value to two decimal places in the box. Include the sign, e.g. -1.23. The box will go green if you are correct. <input class='solveme nospaces' size='10' data-answer='["-7.39"]'/>
...
5.1.  Replace the `NULL`s in the code below to create a logical vector which states TRUE for all values of `nhd` greater than or equal to `d_orig` regardless of sign. **Note:** A logical vector is one that returns TRUE when the expression is true and FALSE when the expression is false. 
